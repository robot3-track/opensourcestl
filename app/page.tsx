      ...
      <div className="w-full h-screen overflow-hidden bg-[#050811] text-slate-100 font-sans relative">
        ...
        {toast && (
          <div className="fixed bottom-6 right-6 z-50 animate-bounce">
            <div
              className={`px-4 py-3 rounded-sm border shadow-2xl flex items-center gap-2.5 text-xs font-sans font-medium ${
                toast.type === "success"
      ...
      <div className="flex flex-col min-h-screen bg-[#040812] text-slate-100 font-sans relative overflow-hidden selection:bg-sky-500/30 selection:text-white h-screen">
        ...
        <header className="flex items-center justify-between px-8 py-5 border-b border-slate-900 bg-[#070b16]/60 backdrop-blur-md z-10 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="bg-slate-900 p-2 rounded-sm text-sky-400 border border-slate-800">
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-1.5 bg-emerald-950/30 border border-emerald-900/40 rounded-sm text-xs font-semibold text-emerald-300 hover:bg-emerald-950/50 transition cursor-pointer"
              >
                {user.photoURL ? (
                  <img src={user.photoURL} alt="Avatar" className="w-4.5 h-4.5 rounded-sm" />
                ) : (
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-8 max-w-5xl mx-auto z-10">
          {/* Welcome Badge */}
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-sm mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
            ...
            <div
              ...
              className="bg-slate-900/40 hover:bg-slate-900/80 border border-slate-850 hover:border-slate-800 rounded-sm p-6 flex flex-col justify-between cursor-pointer transition transform hover:scale-[1.01] hover:shadow-xl group text-left h-[260px]"
            >
              <div>
                <div className="w-10 h-10 bg-slate-950 rounded-sm flex items-center justify-center border border-slate-850 group-hover:border-slate-800 transition mb-4">
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 text-slate-100">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-slate-900 bg-slate-900/40 backdrop-blur-md z-10 flex-shrink-0">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-1.5 hover:bg-slate-800 rounded-sm text-slate-400 hover:text-white border border-slate-800 text-xs font-mono transition cursor-pointer flex items-center gap-1.5"
          >
          ...
          <div className="flex items-center gap-2">
            <div className="bg-slate-900 border border-slate-800 p-1.5 rounded-sm text-indigo-400">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="bg-slate-950 border border-slate-800 rounded-sm px-3 py-1 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-sky-500 font-sans font-medium w-48 text-center"
            title="Project Name"
          />

          <button
            onClick={() => setIsProjectManagerOpen(true)}
            className="p-1.5 bg-slate-900 hover:bg-slate-850 text-slate-300 border border-slate-800 rounded-sm text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
          >
          ...
          <button
            ...
            className="p-1.5 bg-sky-600 hover:bg-sky-500 text-white rounded-sm text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer shadow-md shadow-sky-500/10"
          >
          ...
          {user ? (
            <button
              onClick={() => setIsAuthOpen(true)}
              className="flex items-center gap-2 px-2.5 py-1.5 bg-emerald-950/40 border border-emerald-900/50 hover:bg-emerald-950/60 rounded-sm text-xs font-semibold text-emerald-300 transition cursor-pointer"
            >
              {user.photoURL ? (
                <img src={user.photoURL} alt="Avatar" className="w-4 h-4 rounded-sm" />
              ) : (
                <UserIcon className="w-3.5 h-3.5 text-emerald-400" />
              )}
            ...
            <button
              onClick={() => setIsAuthOpen(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-950/40 border border-indigo-900/50 hover:bg-indigo-950/60 rounded-sm text-xs font-semibold text-indigo-300 transition cursor-pointer"
            >
              <UserIcon className="w-3.5 h-3.5 text-indigo-400" />
              <span>Sign In</span>
            </button>
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-slate-950 border-r border-slate-900 p-4 flex flex-col gap-4 overflow-y-auto flex-shrink-0">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-slate-900 hover:bg-slate-850 border border-slate-800 hover:border-slate-700 rounded-sm text-xs font-medium text-slate-200 transition cursor-pointer"
              >
          ...
          <div className="space-y-2 bg-slate-900/30 border border-slate-900 p-3 rounded-sm">
            <h3 className="text-[10px] text-slate-500 font-mono tracking-widest uppercase">
          ...
          <div className="flex-1 flex flex-col min-h-0 space-y-2">
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 bg-slate-900/20 border border-slate-900 rounded-sm p-2.5">
              {shapes.length === 0 ? (
                ...
                shapes.map((shape) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-2 rounded-sm border transition cursor-pointer group ${
            ...
            {user ? (
              <div className="flex-1 flex flex-col min-h-0 space-y-4">
                ...
                <div className="flex-1 overflow-y-auto space-y-2">
                  {projects.length === 0 ? (
                    <div className="text-center p-6 bg-slate-950 border border-slate-850 rounded-sm">
                    ...
                    projects.map((proj) => (
                      <div
                        ...
                        className="flex items-center justify-between p-3 bg-slate-950/60 hover:bg-slate-950 border border-slate-850 hover:border-slate-800 rounded-sm cursor-pointer transition"
                      >
                        <div className="min-w-0">
                        ...
                        <button
                          ...
                          className="p-1.5 hover:bg-slate-900 rounded-sm text-slate-400 hover:text-rose-400 transition"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
              ...
              <div className="flex flex-col items-center justify-center text-center p-6 bg-slate-950/50 border border-slate-850 rounded-sm">
                ...
                <button
                  ...
                  className="mt-4 bg-gradient-to-r from-sky-600 to-indigo-600 text-white font-semibold text-xs py-2 px-4 rounded-sm shadow-lg hover:scale-[1.02] transition"
                >
                  Authenticate Now
                </button>
              </div>
